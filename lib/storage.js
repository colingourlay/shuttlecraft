...
export const pathToApplications = path.resolve(dataDir, 'applications/');
export const pathToTokens = path.resolve(dataDir, 'tokens/');
...
export const authorizedAccessTokensFile = path.resolve(dataDir, 'authorizedAccessTokens.json');
...
const ensureDataFolder = () => {
  ...
  if (!fs.existsSync(path.resolve(pathToApplications))) {
    logger('mkdir', pathToApplications);
    fs.mkdirSync(path.resolve(pathToApplications), {
      recursive: true
    });
  }
  if (!fs.existsSync(path.resolve(pathToTokens))) {
    logger('mkdir', pathToTokens);
    fs.mkdirSync(path.resolve(pathToTokens), {
      recursive: true
    });
  }
