...
import { readJSONDictionary, writeJSONDictionary, pathToTokens } from './storage.js';

const logger = debug('ono:tokens');

export const authorizedAccessTokens = new Set();
...
export const useCode = token => {
  ...
  authorizedAccessTokens.add(token.accessToken);
  writeJSONDictionary(tokenFile, token);
