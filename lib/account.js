...
export const getAccountForMastodonAPI = (hasSource = false) => {
  const { actor } = getAccount();
  const { url, name, preferredUsername, icon, image } = actor;
  ...
  const account = {
    ...
    note: '',
    avatar: icon.url,
    avatar_static: icon.url,
    header: image.url,
    header_static: image.url,
    locked: false,
    fields: [],
    emojis: [],
    bot: false,
    ...
    last_status_at: null,
    statuses_count: 1,
    followers_count: getFollowers().length,
    following_count: getFollowing().length
  };
